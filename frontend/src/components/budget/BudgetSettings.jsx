...
import {
  ...
  getBudgetCycleConfig, updateBudgetCycleConfig,
} from '../../services/api'
            ...
            <div className="flex gap-1">
              ...
              {pendingDeleteId === acc.id ? (
                <span className="flex gap-1">
                  <button onClick={() => setPendingDeleteId(null)} className="px-1.5 py-0.5 text-xs text-muted hover:text-white rounded">Cancel</button>
                  <button onClick={() => confirmDelete(acc.id)} className="px-1.5 py-0.5 text-xs bg-red-600 hover:bg-red-700 text-white rounded">Delete</button>
                </span>
              ) : (
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex gap-1 flex-shrink-0">
                ...
                {pendingDeleteId === cat.id ? (
                  <span className="flex gap-1">
                    <button onClick={() => setPendingDeleteId(null)} className="px-1.5 py-0.5 text-xs text-muted hover:text-white rounded">Cancel</button>
                    <button onClick={() => confirmDelete(cat.id)} className="px-1.5 py-0.5 text-xs bg-red-600 hover:bg-red-700 text-white rounded">Delete</button>
                  </span>
                ) : (
        ...
        {incomes.map(inc => (
          <div key={inc.id} className="flex items-center justify-between px-3 py-2 bg-surface rounded-lg">
            ...
            <div className="flex gap-1">
              ...
              {pendingDeleteId === inc.id ? (
                <span className="flex gap-1">
                  <button onClick={() => setPendingDeleteId(null)} className="px-1.5 py-0.5 text-xs text-muted hover:text-white rounded">Cancel</button>
                  <button onClick={() => confirmDelete(inc.id)} className="px-1.5 py-0.5 text-xs bg-red-600 hover:bg-red-700 text-white rounded">Delete</button>
                </span>
              ) : (
          ...
          <div key={rule.id} className="flex items-center justify-between px-3 py-2 bg-surface rounded-lg">
            ...
            <div className="flex gap-1 flex-shrink-0">
              ...
              {pendingDeleteId === rule.id ? (
                <span className="flex gap-1">
                  <button onClick={() => setPendingDeleteId(null)} className="px-1.5 py-0.5 text-xs text-muted hover:text-white rounded">Cancel</button>
                  <button onClick={() => confirmDelete(rule.id)} className="px-1.5 py-0.5 text-xs bg-red-600 hover:bg-red-700 text-white rounded">Delete</button>
                </span>
              ) : (
...
function PayPeriodSection() {
  ...
  const handleAdvance = async () => {
    const currentLabel = periodInfo?.current_period?.label || 'the current period'
    if (!window.confirm(
      `Advance from ${currentLabel} to the next pay period?\n\n` +
      `This creates auto-deposits for the new period and cannot be undone without resetting.`
    )) {
      return
    }
    ...
    try {
      ...
      // Hold the button disabled for 3s after a successful advance so a second click
      // can't land before the user has seen the result.
      cooldownRef.current = setTimeout(() => setActionLoading(false), 3000)
    } catch (err) {
      const detail = err?.response?.data?.detail
      const status = err?.response?.status
      setMessage({
        type: 'error',
        text: status === 409 && detail ? detail : 'Failed to advance period',
      })
...
function CycleConfigSection() {
  const [cycleType, setCycleType] = useState('semimonthly')
  ...
  useEffect(() => {
    getBudgetCycleConfig()
      .then((res) => {
        setCycleType(res.data?.cycle_type || 'semimonthly')
        setAnchor(res.data?.anchor || '')
      })
      .catch((err) => console.error('Failed to load cycle config:', err))
      .finally(() => setLoading(false))
  }, [])

  const handleSave = async () => {
    if (cycleType === 'biweekly' && !anchor) {
      setMessage({ type: 'error', text: 'Anchor date is required for biweekly cycles.' })
      return
    }
    if (!window.confirm(
      `Switch budget cycle to "${cycleType}"${cycleType === 'biweekly' ? ` anchored on ${anchor}` : ''}?\n\n` +
      `New transactions will be tagged with the new cycle's period key. Historical snapshots are not rewritten.`
    )) {
      return
    }
    setSaving(true)
    setMessage(null)
    try {
      const res = await updateBudgetCycleConfig({
        cycle_type: cycleType,
        anchor: cycleType === 'biweekly' ? anchor : null,
      })
      setCycleType(res.data?.cycle_type || 'semimonthly')
      setAnchor(res.data?.anchor || '')
      setMessage({ type: 'success', text: 'Cycle config updated. Refresh the Budget page.' })
    } catch (err) {
      const detail = err?.response?.data?.detail
      setMessage({ type: 'error', text: detail || 'Failed to update cycle config' })
    } finally {
      setSaving(false)
    }
  }

  if (loading) return <div className="animate-pulse bg-surface rounded-xl h-32 mb-4" />

  return (
    <div className="p-6 rounded-xl mb-4" style={{ backgroundColor: 'var(--color-surface)' }}>
      <h3 className="text-lg font-semibold mb-3" style={{ color: 'var(--color-text-primary)' }}>Cycle Type</h3>
      <p className="text-sm mb-4" style={{ color: 'var(--color-text-muted)' }}>
        Semi-monthly splits each month 1st-14th and 15th-end. Biweekly rolls every 14 days from the anchor date.
      </p>
      <div className="flex gap-4 mb-4">
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            name="cycle-type"
            value="semimonthly"
            checked={cycleType === 'semimonthly'}
            onChange={(e) => setCycleType(e.target.value)}
          />
          <span style={{ color: 'var(--color-text-primary)' }}>Semi-monthly</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="radio"
            ...
            onChange={(e) => setCycleType(e.target.value)}
          />
          <span style={{ color: 'var(--color-text-primary)' }}>Biweekly</span>
        </label>
      </div>
      {cycleType === 'biweekly' && (
        <div className="mb-4">
          <label className="block text-sm mb-1" style={{ color: 'var(--color-text-secondary)' }}>
            Anchor date (start of a reference cycle)
          </label>
          <input
            type="date"
            value={anchor}
      ...
      {message && (
        <div className={`text-sm mb-3 ${message.type === 'success' ? 'text-green-500' : 'text-red-500'}`}>
          {message.text}
        </div>
      )}
      <button
        ...
        style={{ backgroundColor: 'var(--color-blue-600)', color: 'white' }}
      >
        {saving ? 'Saving...' : 'Save Cycle Config'}
      </button>
    </div>
  )
}
