...
import { getBudgetPeriodSummary, getBudgetCategories, updateBudgetCategory, getAccountsWithBalances, getBudgetPeriodReference, getBudgetCycleConfig } from '../../services/api'
...
function biweeklyCycleContaining(refDate, anchor) {
  // Day diff via UTC-normalized midnight avoids DST-crossing rounding bugs
  // where a spring-forward/fall-back between the two dates would add or subtract
  // an hour and push Math.floor off by one.
  const utcRef = Date.UTC(refDate.getFullYear(), refDate.getMonth(), refDate.getDate())
  const utcAnc = Date.UTC(anchor.getFullYear(), anchor.getMonth(), anchor.getDate())
  const diffDays = Math.floor((utcRef - utcAnc) / (24 * 60 * 60 * 1000))
  const cycleN = Math.floor(diffDays / 14)
  const start = addDays(anchor, cycleN * 14)
  const end = addDays(start, 13)
  return { start, end }
}

// Compute 3 fixed periods.
// refDate = target period reference
// advanceDate = when the advance was done, or null for default periods
// cycleConfig = { cycle_type, anchor } from /budget/cycle-config/
function computePeriods(refDate, advanceDate, cycleConfig) {
  const cfg = cycleConfig || { cycle_type: 'semimonthly', anchor: null }
  const mn = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']

  if (cfg.cycle_type === 'biweekly' && cfg.anchor) {
    const anchor = new Date(cfg.anchor + 'T12:00:00')
    const cur = biweeklyCycleContaining(refDate, anchor)
    const prev = biweeklyCycleContaining(addDays(cur.start, -1), anchor)
    const curStart = fmtDate(cur.start.getFullYear(), cur.start.getMonth() + 1, cur.start.getDate())
    const curEnd = fmtDate(cur.end.getFullYear(), cur.end.getMonth() + 1, cur.end.getDate())
    const prevStart = fmtDate(prev.start.getFullYear(), prev.start.getMonth() + 1, prev.start.getDate())
    const prevEnd = fmtDate(prev.end.getFullYear(), prev.end.getMonth() + 1, prev.end.getDate())
    const thisPeriod = { start: curStart, end: curEnd, label: `This Period (${fmtRange(curStart, curEnd)})` }
    const lastPeriod = { start: prevStart, end: prevEnd, label: `Last Period (${fmtRange(prevStart, prevEnd)})` }
    const ry = refDate.getFullYear(), rm = refDate.getMonth() + 1
    const thisMonth = {
      start: fmtDate(ry, rm, 1),
      end: fmtDate(ry, rm, lastDayOf(ry, rm)),
      label: mn[rm - 1],
    }
    return [thisPeriod, lastPeriod, thisMonth]
  }

  // Semi-monthly (original behavior)
  const ry = refDate.getFullYear(), rm = refDate.getMonth() + 1
...
function BudgetOverview() {
  ...
  // Fetch reference date + advance date + cycle config, compute periods
  useEffect(() => {
    const init = async () => {
      let refDate = new Date()
      let advanceDate = null
      let cycleConfig = { cycle_type: 'semimonthly', anchor: null }
      try {
        const [refRes, cfgRes] = await Promise.all([
          getBudgetPeriodReference(),
          getBudgetCycleConfig().catch(() => ({ data: cycleConfig })),
        ])
        if (refRes.data?.reference_date) {
          refDate = new Date(refRes.data.reference_date + 'T12:00:00')
        }
        if (refRes.data?.advance_date) {
          advanceDate = new Date(refRes.data.advance_date + 'T12:00:00')
        }
        if (cfgRes?.data) {
      ...
      setPeriods(computePeriods(refDate, advanceDate, cycleConfig))
      setInitialized(true)
    }
    init()
  }, [])
