...
// Sortable Standard Task Item Component
function SortableStandardTaskItem({
  task,
  idx,
  standardTasksLength,
  isEditing,
      ...
      {isEditing ? (
        <>
          <input
            ...
            onChange={(e) => onTitleChange({ ...editingStandardTask, title: e.target.value })}
...
function WorkerVisitTasksTab({ worker }) {
  ...
  // DND Kit sensors for standard tasks drag and drop
  ...
  const handleAddStandardTask = async () => {
    ...
    try {
      ...
      // Auto-populate to current visit if one exists
      if (currentVisit) {
        try {
          const visitMaxOrder = Math.max(0, ...currentVisit.tasks.map(t => t.sort_order))
          await addWorkerVisitTask(worker.id, currentVisit.id, {
            title: taskTitle,
            sort_order: visitMaxOrder + 1,
            is_standard: true
          })
        } catch (err) {
          console.error('Failed to add task to current visit:', err)
        }
      }
  ...
  const handleReorderStandardTask = async (taskId, direction) => {
    if (saving) return

    const taskIdx = standardTasks.findIndex(t => t.id === taskId)
    if (taskIdx === -1) return

    let newIdx = taskIdx
    if (direction === 'up' && taskIdx > 0) {
      newIdx = taskIdx - 1
    } else if (direction === 'down' && taskIdx < standardTasks.length - 1) {
      newIdx = taskIdx + 1
    } else {
    ...
    setSaving(true)
    try {
      const reorderedTasks = [...standardTasks]
      const [movedTask] = reorderedTasks.splice(taskIdx, 1)
      reorderedTasks.splice(newIdx, 0, movedTask)

      // Update local state immediately for responsiveness
      setStandardTasks(reorderedTasks)

      // Save to backend
      await reorderWorkerStandardTasks(worker.id, reorderedTasks.map(t => t.id))
    } catch (err) {
      console.error('Failed to reorder standard task:', err)
  ...
  const handleDragEndStandardTask = async (event) => {
    const { active, over } = event

    if (over && active.id !== over.id) {
      const oldIndex = standardTasks.findIndex(t => t.id.toString() === active.id)
      const newIndex = standardTasks.findIndex(t => t.id.toString() === over.id)

      if (oldIndex !== -1 && newIndex !== -1) {
        setSaving(true)
        try {
          const reorderedTasks = arrayMove(standardTasks, oldIndex, newIndex)
          setStandardTasks(reorderedTasks)
          await reorderWorkerStandardTasks(worker.id, reorderedTasks.map(t => t.id))
        } catch (err) {
          console.error('Failed to reorder standard task:', err)
  ...
  // Reorder visit tasks
  const handleReorderVisitTask = async (taskId, direction) => {
    if (!currentVisit || saving) return

    const taskIdx = activeTasks.findIndex(t => t.id === taskId)
    if (taskIdx === -1) return

    let newIdx = taskIdx
    if (direction === 'up' && taskIdx > 0) {
      newIdx = taskIdx - 1
    } else if (direction === 'down' && taskIdx < activeTasks.length - 1) {
      newIdx = taskIdx + 1
    } else {
      return // Can't move beyond bounds
    }
    ...
    try {
      ...
      const [movedTask] = reorderedActiveTasks.splice(taskIdx, 1)
      reorderedActiveTasks.splice(newIdx, 0, movedTask)

      // Reconstruct full tasks list with backlog and completed
      ...
      // Update local state immediately for responsiveness
      setCurrentVisit({ ...currentVisit, tasks: reorderedTasks })

      // Save to backend
      await reorderWorkerVisitTasks(worker.id, currentVisit.id, reorderedTasks.map(t => t.id))
    } catch (err) {
      console.error('Failed to reorder visit task:', err)
      await loadData() // Reload on error
    } finally {
      setSaving(false)
    }
  ...
  const allTasksComplete = activeTasks.length === 0 && completedTasks.length > 0
      ...
      {showStandardSettings && (
        <div className="rounded-xl p-4" style={{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-border-default)' }}>
          ...
          <DndContext
            ...
            onDragEnd={handleDragEndStandardTask}
          >
            <div className="space-y-2 mb-4">
              <SortableContext
                items={standardTasks.map(t => t.id.toString())}
                strategy={verticalListSortingStrategy}
              >
                {standardTasks.map((task, idx) => (
                  <SortableStandardTaskItem
                    key={task.id}
                    task={task}
                    idx={idx}
                    standardTasksLength={standardTasks.length}
                    isEditing={editingStandardTask?.id === task.id}
                    editingStandardTask={editingStandardTask}
                    onEdit={setEditingStandardTask}
                    onCancel={() => setEditingStandardTask(null)}
                    onUpdate={handleUpdateStandardTask}
                    onDelete={handleDeleteStandardTask}
                    onReorder={handleReorderStandardTask}
                    onTitleChange={setEditingStandardTask}
                  ...
                  >
                    ...
                    {editingVisitTaskId !== task.id && (
                      <>
                        ...
                        {idx > 0 && (
                          <button
                            onClick={() => handleReorderVisitTask(task.id, 'up')}
                            className="p-1 text-muted hover:text-blue-400"
                            title="Move up"
                            disabled={saving}
                          >
                        ...
                        {idx < activeTasks.length - 1 && (
                          <button
                            onClick={() => handleReorderVisitTask(task.id, 'down')}
                            className="p-1 text-muted hover:text-blue-400"
                            title="Move down"
                            disabled={saving}
                          >
