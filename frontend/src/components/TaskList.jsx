...
function TaskList({ tasks, title, onTaskToggle, showDate = false, showTimeAndLocation = false, maxVisibleCompleted = 10, className = '', fillContainer = false }) {
  ...
  // Optimistic completion overrides: {taskId: optimistic_is_completed}.
  // Lets the checkbox flip instantly while the parent's refetch is in flight,
  // so the user does not have to refresh to see their click register.
  const [pendingState, setPendingState] = useState({})

  // Drop overrides whose optimistic value now matches the server state in `tasks`.
  useEffect(() => {
    setPendingState(prev => {
      const next = { ...prev }
      let changed = false
      for (const t of tasks || []) {
        if (t.id in next && next[t.id] === t.is_completed) {
          delete next[t.id]
          changed = true
        }
      }
  ...
  const handleToggle = async (task) => {
    ...
    const optimistic = !task.is_completed
    setPendingState(prev => ({ ...prev, [task.id]: optimistic }))
  ...
  // Apply optimistic overrides so the partition reflects pending toggles instantly.
  const viewTasks = tasks.map(t =>
    t.id in pendingState ? { ...t, is_completed: pendingState[t.id] } : t
  )

  // Separate completed and incomplete tasks
  const incompleteTasks = viewTasks.filter(t => !t.is_completed)
  const completedTasks = viewTasks.filter(t => t.is_completed)
