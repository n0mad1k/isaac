...
PI_USER="${PI_USER:-pi}"
SSH_KEY="${SSH_KEY:-$HOME/.ssh/isaac}"
...
echo "[3/6] Syncing backend files..."
rsync -avz --delete \
    -e "ssh -i $SSH_KEY" \
    --exclude '__pycache__' \
    --exclude '*.pyc' \
...
echo "[4/6] Syncing frontend files..."
rsync -avz --delete \
    -e "ssh -i $SSH_KEY" \
...
echo "[5/6] Syncing deployment files..."
rsync -avz \
    -e "ssh -i $SSH_KEY" \
    "$LOCAL_DIR/deploy/" \
...
echo ""
echo "Next steps:"
echo "1. SSH into the Pi: ssh -i ~/.ssh/isaac $PI_USER@$PI_HOST"
