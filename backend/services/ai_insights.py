...
async def generate_weekly_budget_review():
    ...
    try:
        ...
        prompt = (
            ...
            "- 'BUDGET VS ACTUAL' shows SPENDING categories - these are expenses\n"
            "- 'SAVINGS ALLOCATIONS' shows TRANSFER categories - these are automatic deposits to accounts\n"
            "- These are SEPARATE - spending more or less does NOT affect savings allocations\n"
            "- Never suggest that reducing spending would 'preserve' or 'help' a savings allocation\n\n"
            "Format your response as:\n"
            "- **Categories over budget:** (only if explicitly marked OVER in the data, else 'None')\n"
            "- **Categories near their limit (80%+ spent):** (only if 'near limit' in status, else 'None')\n"
            "- **Actionable saving opportunity:** One tip based on recent transactions in SPENDING categories only\n"
            "- **Status:** Brief summary of overall budget health\n"
