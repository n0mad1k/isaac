...
class CalendarSyncService:
    ...
    def vevent_to_task_dict(self, component, component_type: str = None) -> Dict[str, Any]:
        ...
        isaac_task_id = component.get('x-isaac-task-id')
        if isaac_task_id:
            task_dict['isaac_task_id'] = int(str(isaac_task_id))
    ...
    async def sync_all_tasks_to_calendar(self, db: AsyncSession, calendar_uids: set = None) -> int:
        ...
        for task in tasks:
            # Skip tasks that were imported from phone (have a non-isaac UID)
    ...
    async def sync_calendar_to_tasks(self, db: AsyncSession) -> dict:
        ...
        for event_dict in events:
            ...
            if calendar_uid.startswith('isaac-task-') or event_dict.get('isaac_task_id'):
