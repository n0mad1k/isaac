...
class EmailService:
    ...
    async def send_email(
        ...
        Args:
            ...
            subject_prefix: Override the default [farm_name] prefix
            from_name: Override the From header display name (defaults to farm name)
        """
        ...
        try:
            ...
            # Use farm_name from settings as default prefix
            prefix_name = subject_prefix or self.farm_name
            prefix = f"[{_sanitize_header(prefix_name)}]"
            message["Subject"] = f"{prefix} {_sanitize_header(subject)}"

            # Use from_name if provided, else farm_name, to override the From header display name
            display_name = from_name or self.farm_name
            ...
            message["From"] = f"{_sanitize_header(display_name)} <{actual_email}>"
    ...
    async def send_daily_digest(
        ...
        subject = f"Daily Digest - {datetime.now().strftime('%m/%d/%Y')}"
        ...
        <body>
            <div class="header">
                <h1>🌾 {_escape_html(self.farm_name)} Daily Digest</h1>
        ...
        html += f"""
            <div class="section" style="text-align: center; color: #666; font-size: 12px;">
                <p>Generated by {_escape_html(self.farm_name)}</p>
            </div>
        </body>
        </html>
        """
